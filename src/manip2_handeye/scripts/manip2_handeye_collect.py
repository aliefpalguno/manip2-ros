...
tf_buffer = None  # global TF buffer
...
def transform_to_matrix(transform):
    ...
    T = np.dot(T, tr.quaternion_matrix((q.x, q.y, q.z, q.w)))
    return T


def capture_cb(req):
    global R_g2b_list, t_g2b_list, R_t2c_list, t_t2c_list, tf_buffer
    ...
    if tf_buffer is None:
        return TriggerResponse(
            success=False,
            message="TF buffer not initialized."
        )

    # Get parameters
    base_frame  = rospy.get_param("~base_frame", "base_link")
    ee_frame    = rospy.get_param("~ee_frame", "end_effector")
...
def main():
    global tf_buffer
    rospy.init_node("manip2_handeye_collect")
    ...
    rospy.loginfo("manip2_handeye_collect ready. Services: 'capture', 'save'.")
    rospy.spin()
